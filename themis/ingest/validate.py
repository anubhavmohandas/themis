...
def validate_rows(rows: list[dict], mapping: dict, chain_id: str | None, dedupe: bool = True) -> dict:
    ...
    Returns `valid_rows` with, aligned to it, `valid_chains` (each row's chain id)."""
    ...
    valid_rows, valid_chains, rejected = [], [], []
    ...
    return dict(valid_rows=valid_rows, valid_chains=valid_chains,
