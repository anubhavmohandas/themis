...
class TestVerdicts(unittest.TestCase):
    def verdict(self, **kw):
        return A.assess(result(**kw))["verdict"]

    def test_clean_evidence_is_trustworthy(self):
        self.assertEqual(self.verdict(conflict=0, agree=80), A.TRUSTWORTHY)
    ...
    def test_measured_problems_are_not_trustworthy(self):
        self.assertEqual(self.verdict(conflict=20, agree=60), A.NOT_TRUSTWORTHY)           # 25% of judged pairs conflict
        self.assertEqual(self.verdict(contradictions=5, conflict=0, agree=80), A.NOT_TRUSTWORTHY)
        self.assertEqual(self.verdict(n_rej=30, conflict=0, agree=80), A.NOT_TRUSTWORTHY)

    def test_no_external_evidence_is_never_trust(self):
        ...
        self.assertEqual(self.verdict(comparable=5, agree=5, conflict=0, contradictions=5), A.NOT_TRUSTWORTHY)

    def test_a_stopped_dataset_cannot_be_assessed_and_says_why(self):
        a = A.assess(dict(stopped=True, message="Unsupported dataset.\n\nMore text."))
        self.assertEqual(a["verdict"], A.CANNOT_ASSESS)
    ...
    def test_the_statement_carries_the_verdict_and_its_measured_reasons(self):
        ...
        self.assertTrue(s.startswith("According to the THEMIS report, this dataset is NOT TRUSTWORTHY, because:"))
